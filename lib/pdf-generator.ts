...
import type { Empresa, Cliente, LineaFactura, LineaPresupuesto, Factura } from '@/types'
...
// ===========================================
// PDF DE MÚLTIPLES FACTURAS (BULK) OPTIMIZADO
// ===========================================
...
export async function generateBulkInvoicePDF(data: BulkInvoiceData): Promise<Buffer> {
  const { facturas, empresa } = data
  const color = empresa.color_primario || "#1e40af"
  ...
  for (let idx = 0; idx < facturas.length; idx++) {
    const { factura, cliente } = facturas[idx]
